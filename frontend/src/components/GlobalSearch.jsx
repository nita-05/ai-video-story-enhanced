...
const GlobalSearch = ({ isOpen, onClose }) => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Seed stats (views/likes) for each result
        try {
          ...
          console.warn('Failed to seed stats:', e);
        }
      } else {
      ...
      // Enhanced demo results with better data
      setSearchResults([
        {
          id: '1',
  ...
  const handleTrendingClick = (trend) => {
    ...
    // Auto-search when clicking trending terms
  ...
  const handleWatchNow = (videoId) => {
    ...
    // Also increment view count in backend and reflect in list
  ...
  const handleLike = async (videoId) => {
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        // Update the search results with new like count
    ...
    >
      <div className="space-y-8">
        ...
        >
          ...
          >
            <motion.div 
              className="w-24 h-24 bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 rounded-full flex items-center justify-center mx-auto mb-6 shadow-2xl"
          ...
          <motion.h2 
            className="text-4xl font-bold bg-gradient-to-r from-gray-900 via-blue-800 to-indigo-900 bg-clip-text text-transparent mb-3"
        ...
        >
          <div className="relative group">
            <div className="absolute inset-0 bg-gradient-to-r from-blue-600 to-indigo-700 rounded-2xl blur opacity-20 group-hover:opacity-30 transition-opacity duration-300"></div>
            <div className="relative bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
              <div className="absolute inset-0 bg-gradient-to-r from-blue-500/5 to-indigo-500/5 opacity-0 group-focus-within:opacity-100 transition-opacity duration-300"></div>
              <div className="relative flex items-center p-2">
                <motion.div
                  animate={{ scale: [1, 1.1, 1] }}
                  transition={{ duration: 2, repeat: Infinity }}
                >
                  <Search className="ml-4 w-6 h-6 text-gray-400 group-focus-within:text-blue-500 transition-colors duration-300" />
                </motion.div>
                <input
                  type="text"
                  value={searchQuery}
                  ...
                  className="flex-1 px-4 py-4 text-lg border-none outline-none bg-transparent placeholder-gray-400 focus:placeholder-gray-300 transition-colors duration-300"
                />
                <div className="flex gap-3 mr-2">
                  ...
                  <motion.button
                    ...
                    className="px-6 py-2 bg-gradient-to-r from-blue-600 to-indigo-700 text-white rounded-xl hover:from-blue-700 hover:to-indigo-800 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 font-medium shadow-lg hover:shadow-xl flex items-center gap-2 relative overflow-hidden"
        ...
        >
          ...
          >
            ...
            <p className="text-gray-600">Discover what others are searching for</p>
          </motion.div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {trendingSearches.map((trend, index) => (
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                className="group bg-gradient-to-r from-gray-50 to-blue-50 hover:from-blue-50 hover:to-indigo-50 p-4 rounded-xl border border-gray-200 hover:border-blue-300 transition-all duration-300 hover:shadow-lg relative overflow-hidden"
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              >
                ...
                <div className="relative z-10">
                  ...
                  <div className="text-sm font-medium text-gray-800 group-hover:text-blue-800 transition-colors">
                    {trend.term}
                  </div>
        ...
        <AnimatePresence>
          {showFilters && (
            <motion.div
              ...
              className="max-w-5xl mx-auto bg-gradient-to-r from-gray-50 to-blue-50 rounded-2xl p-6 border border-gray-200 shadow-lg"
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent transition-all"
                  >
          ...
          <div className="max-w-7xl mx-auto">
            <div className="flex items-center justify-between mb-8">
              <div>
                ...
                <p className="text-gray-600">Discovering stories from around the world</p>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {searchResults.map((result, index) => (
                <motion.div
                  ...
                  className="bg-white border border-gray-200 rounded-2xl overflow-hidden hover:shadow-xl transition-all duration-300 cursor-pointer group hover:-translate-y-1"
                >
                  {/* Enhanced Video Thumbnail */}
                  <div className="relative h-48 bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-100 flex items-center justify-center text-6xl group-hover:scale-105 transition-transform duration-300 overflow-hidden">
                    {result.thumbnail ? (
                      <img 
                    ...
                    <div className="absolute top-3 left-3">
                      <span className="px-2 py-1 bg-black/70 text-white rounded-lg text-xs font-medium flex items-center gap-1">
                        {getCategoryIcon(result.category)} {result.category || 'general'}
                      </span>
                    </div>
                    <div className="absolute top-3 right-3">
                      <span className={`px-2 py-1 rounded-lg text-xs font-medium border ${getRelevanceColor(result.relevance || 0)}`}>
                  ...
                  <div className="p-5">
                    ...
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium text-gray-700">{result.user || 'You'}</span>
                      </div>
                    ...
                    <div className="flex flex-wrap gap-2 mb-4">
                      {(result.tags || []).slice(0, 4).map((tag, tagIndex) => (
                        <span
                          key={tagIndex}
                          className="px-3 py-1 bg-gradient-to-r from-blue-50 to-indigo-50 text-blue-700 rounded-full text-xs font-medium border border-blue-200 hover:from-blue-100 hover:to-indigo-100 transition-colors"
                        >
                          {tag}
                        </span>
                      ))}
                    ...
                    <div className="flex gap-2">
                      <button 
                        ...
                        className="flex-1 px-4 py-2 bg-gradient-to-r from-blue-600 to-indigo-700 text-white rounded-xl hover:from-blue-700 hover:to-indigo-800 transition-all duration-300 text-sm font-medium flex items-center justify-center gap-2 shadow-md hover:shadow-lg group-hover:scale-105"
                      >
                      ...
                      <button 
                        ...
                        className="px-4 py-2 bg-gray-100 text-gray-700 rounded-xl hover:bg-gray-200 transition-all duration-300 hover:scale-105"
                      >
          ...
          <div className="text-center py-12">
            ...
            <button
              onClick={() => setSearchQuery('')}
                             className="px-4 py-2 bg-gradient-to-r from-blue-600 to-indigo-700 text-white rounded-lg hover:from-blue-700 hover:to-indigo-800 transition-colors shadow-sm"
            >
              Clear Search
            </button>
          </div>
        ...
        {/* Vision Statement */}
                 <div className="max-w-4xl mx-auto bg-gradient-to-r from-blue-50 via-indigo-50 to-blue-100 p-6 rounded-xl border border-blue-200 shadow-sm">
           <div className="flex items-center gap-3 mb-3">
             <Sparkles className="w-6 h-6 text-blue-600" />
             <h4 className="font-semibold text-blue-900">The Power of Collective Memory</h4>
           </div>
