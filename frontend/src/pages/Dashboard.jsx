...
const Dashboard = () => {
  ...
  const handleGenerateStory = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // Optional: start narration synced with video
        try {
          const video = videoElementRef?.current;
          const narrationText = response.data.fullNarration || '';
          if (video && window.speechSynthesis && narrationText) {
            // Stop any previous speech
            window.speechSynthesis.cancel();
            // Chunk long narration into ~180-220 char pieces for Web Speech stability
            const chunks = [];
            let remaining = narrationText.trim();
            const maxLen = 200;
            ...
            let timers = [];
            // Clear timers when video paused or ended
            const clearTimers = () => { timers.forEach(t => clearTimeout(t)); timers = []; window.speechSynthesis.cancel(); };
            video.removeEventListener('pause', clearTimers);
            video.removeEventListener('ended', clearTimers);
            video.addEventListener('pause', clearTimers);
            video.addEventListener('ended', clearTimers);
            // Ensure playback starts
            if (video.paused) await video.play().catch(() => {});
            // Start narration immediately and keep speaking chunks back-to-back
            let delay = 0;
            chunks.forEach((c, idx) => {
              timers.push(setTimeout(() => speakChunk(c), delay));
              // Estimate next chunk delay by approximate speaking rate (~150 wpm ≈ 10 chars/sec at this size)
              const approxMs = Math.max(1500, Math.min(6000, c.length * 70));
              delay += approxMs;
            });
          }
        } catch (e) {
          console.warn('Narration start failed:', e);
        }
      } else {
  ...
  return (
    <div className="min-h-screen relative overflow-hidden" style={{
      ...
      <nav className="fixed top-0 left-0 right-0 bg-white/10 backdrop-blur-md border-b border-white/20 z-50 shadow-lg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-8">
              ...
              >
                <div className="w-8 h-8 bg-gradient-to-r from-blue-600 to-indigo-700 rounded-lg flex items-center justify-center shadow-lg hover:shadow-blue-500/25 transition-all duration-300">
                  <Video className="w-5 h-5 text-white" />
                </div>
                <span className="text-xl font-bold text-white hover:text-blue-200 transition-colors duration-300">
                  Footage Flow
                </span>
              </motion.div>
              ...
              <div className="hidden md:flex items-center space-x-6">
                <motion.button 
                  ...
                  className="text-sm font-medium text-white/80 hover:text-white transition-all duration-300 relative group cursor-pointer flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-white/10"
                ...
                >
                  ...
                  <span>Render</span>
                  <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-gradient-to-r from-red-400 to-pink-400 transition-all duration-300 group-hover:w-full"></span>
                </motion.button>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-4">
              <motion.div 
                className="flex items-center space-x-2 px-3 py-2 rounded-lg bg-white/5 backdrop-blur-sm border border-white/10"
              ...
              <motion.button 
                ...
                className="flex items-center space-x-2 px-4 py-2 text-sm font-medium text-white/80 hover:text-white transition-all duration-300 bg-white/5 hover:bg-red-500/20 rounded-lg border border-white/10 hover:border-red-400/30"
      ...
      <motion.div 
        ...
        className="pt-20 pb-8 text-center relative"
      >
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <p className="text-xl text-blue-100 max-w-2xl mx-auto">
              Transform your videos with AI-powered transcription, storytelling, and analysis
            </p>
          </motion.div>
          <motion.div 
            animate={{ y: [0, 10, 0] }}
      ...
      <div className="max-w-6xl mx-auto px-6 sm:px-8 lg:px-10 pb-24 pt-8">
        <div className="space-y-16">
          ...
          <motion.div 
            ...
            viewport={{ once: true }}
            className="relative bg-white/10 backdrop-blur-md rounded-xl shadow-xl border border-white/20 p-8 overflow-hidden"
          >
            ...
            <div className="mb-6">
              <div className="flex items-center gap-3 mb-3">
                <motion.div 
                  whileHover={{ scale: 1.05 }}
                  className="w-8 h-8 bg-gradient-to-r from-blue-600 to-indigo-700 rounded-lg flex items-center justify-center shadow-md"
            ...
            <div className="space-y-6">
              ...
              >
                <label className="block">
                  <div className="border-2 border-dashed border-white/30 rounded-xl p-8 text-center hover:border-blue-400 hover:bg-white/5 transition-all duration-300 cursor-pointer group relative overflow-hidden">
                    ...
                    {preview ? (
                      <div className="w-full h-full relative z-10">
                        <video 
                          ...
                          src={preview.url} 
                          controls 
                          className="w-full h-full rounded-lg shadow-lg max-h-48 mx-auto border border-white/20" 
                        />
                      ...
                      <div className="relative z-10">
                        <motion.div
                          ...
                          transition={{ duration: 2, repeat: Infinity }}
                        >
                          <Upload className="w-12 h-12 text-white/60 mx-auto mb-6 group-hover:text-blue-300 transition-colors" />
                        </motion.div>
                        <div className="text-lg font-medium text-white mb-2 group-hover:text-blue-200 transition-colors">
                        ...
                        <div className="mt-4 flex justify-center">
                          <div className="w-16 h-1 bg-gradient-to-r from-blue-400 to-indigo-400 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                        </div>
                      </div>
                    )}
                  </div>
              ...
              <div className="flex justify-center pt-4">
                <motion.button 
                  ...
                  className="px-8 py-3 bg-gradient-to-r from-blue-600 to-indigo-700 text-white rounded-xl font-semibold hover:from-blue-700 hover:to-indigo-800 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 shadow-lg hover:shadow-xl"
                >
          ...
          <div className="mb-4">
            {videoId && (
              <button
                ...
                className="w-full px-6 py-3 bg-gradient-to-r from-blue-500 via-blue-600 to-indigo-700 text-white rounded-lg hover:from-blue-600 hover:via-blue-700 hover:to-indigo-800 disabled:bg-gray-400 disabled:cursor-not-allowed transition-all duration-300 font-semibold shadow-md hover:shadow-lg transform hover:-translate-y-0.5"
